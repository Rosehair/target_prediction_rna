"""
This file was not made by me, it was decompiled from deepTarget's bio_utils.pyc lib file
"""
...
embd_model_file = '._deepTarget_embd_model.json'
model_file = '._deepTarget_model.json'
embd_model_weight_file = '._deepTarget_weights_embd_model.hdf5'
model_weight_file = '._deepTarget_weights_model_fold1.hdf5'
...
def read_fasta(mirna_fasta_file, mrna_fasta_file):
    ...
    return (
     mirna_ids, mirna_sequences, mrna_ids, mrna_sequences)
...
def nucleotide_to_int(nucleotide, max_len):
    ...
    ints_enc = [0 for i in range(max_len)]
...
def get_statistics(confusion_mat):
    ...
    return (
     sensitivity, specificity, f1_score, ppv, npv)
...
def make_train_pair(mirna_ids, mirna_seqs, mrna_ids, mrna_seqs, query_ids, target_ids, labels):
    ...
    return (
     X_query_ids, X_query_seqs, X_target_ids, X_target_seqs, Y)
...
def make_test_pair(mirna_ids, mirna_seqs, mrna_ids, mrna_seqs):
    ...
    return (
     X_query_ids, X_query_seqs, X_target_ids, X_target_seqs)
...
def read_label(label_file):
    ...
    labels = list((int(lines[l]) for l in range(len(lines))))
    return labels


def label_to_list(labels):
    return list(([int(lines[l])] for l in range(len(lines))))
